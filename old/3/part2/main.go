...
func parseWire(path []string) Wire {
	...
	for _, instruction := range path {
		...
		next := prev.apply(direction, distance)
...
func intersection(w1 Wire, w2 Wire) Wire {
	w1set := make(map[string]WirePoint, len(w1))
	for _, p := range w1 {
		w1set[p.hash()] = p
	}

	intersections := make(Wire, 0)

	for _, p1 := range w2 {
		if p2, ok := w1set[p1.hash()]; ok {
			intersections = append(intersections, WirePoint{Point{p1.x, p1.y}, p1.ordinal + p2.ordinal})
		}
	}
